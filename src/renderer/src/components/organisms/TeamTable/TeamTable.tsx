...
const TeamTable: React.FC = () => {
  ...
  const handleAddTeam = (event: FormEvent) => {
    ...
    if (
      teams.some((team: { name: string }) => team.name.toLowerCase() === newTeamName.toLowerCase())
    ) {
  ...
  const handleUpdateScore = (id: number, mod: number) => {
    const teamToUpdate = teams.find((team: Team) => team.id === id)
    if (teamToUpdate) {
        ...
        <div>
          <Button
            ...
            onClick={() => {
              ...
              const prevIndex =
                currentIndex === -1 ? 0 : (currentIndex - 1 + teams.length) % teams.length
