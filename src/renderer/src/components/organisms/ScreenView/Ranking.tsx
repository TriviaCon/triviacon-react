      ...
      {sortedTeams.length === 0 ? (
        ...
        <>
          {sortedTeams.map((team, index) => {
            let fontSize;
            if (index === 0) fontSize = '4.5rem'; // 1st place
            else if (index === 1) fontSize = '3.5rem'; // 2nd place
            else if (index === 2) fontSize = '2.9rem'; // 3rd place
            ...
            else if (index === 1) displayText = `🥈 ${index + 1}. ${team.name}: ${team.score} points 🥈`;
            else if (index === 2) displayText = `🥉 ${index + 1}. ${team.name}: ${team.score} points 🥉`;
            else displayText = `${index + 1}. ${team.name}: ${team.score} points`;
            return (
              <React.Fragment key={team.id}>
                <HeadingTag style={{ fontSize, margin: '0.5em 0' }}>
                  {displayText}
                </HeadingTag>
                {/* Insert spacing after the 3rd team if there are more than 3 teams */}
                {index === 2 && sortedTeams.length > 3 && (
